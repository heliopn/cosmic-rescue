...
```bash
cosmic-rescue$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
cosmic-rescue$ sam local start-api
cosmic-rescue$ curl http://localhost:3000/classify_digit
```
...
```bash
cosmic-rescue$ sam logs -n InferenceFunction --stack-name "cosmic-rescue" --tail
```
